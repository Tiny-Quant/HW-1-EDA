...
Groups 1, 2, and 4 were mainly geographically tied.
Cluster 1 contain most of the listings from Staten Island and Brooklyn,
...
all located in Manhattan, but are rarely actually booked. Finally, Cluster 3 was
...
caption <- "Quantity and Pattern of Missing Data. The blue bars represent
    the quantity of the pattern shone of the matrix below. The black bar
    show the number of missing values of the variable to the right across
    all patterns."
```
...
with the highest total number and frequency of reviews. With a median
...
mean the listing is book often or it could mean the host does not offer bookings
often. A medium availability might actually be the best indicator of good business
as the Airbnb could be offered and book frequently on a rolling basis.
...
\textbf{Figure 5} gives more color on the pricing in cluster 3 as it is mostly
comprised of private room instead of entire homes and apartments.
...
Unlike all of the other clusters, cluster 3 is geographically disperse across
the 5 boroughs. This can be seen in \text{Figure 1} and \textbf{Figure 6}.
...
corporate of investor owned properties. This might also explain why
\textbf{Figure 5} shows that all the listings in cluster 5 are for the entire
...
Single property owners rent out a room to make some extra cash. Geographically
...
they require larger bookings. Although not booked often, they appear to make
...
instead of the individual it was originally marketed to.
